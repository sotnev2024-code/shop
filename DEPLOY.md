...
## 3. Альтернатива: режим разработки на сервере

Если нужен один конфиг с поднятием всего (включая dev-сервер фронта и отдельный nginx):

```bash
docker compose up -d --build
```

- Сайт: **порт 80** (nginx проксирует на фронт и бэкенд).
- Бэкенд: порт 8000, фронт dev: порт 3000 (внутри сети).
- Для продакшена предпочтительно использовать `docker-compose.prod.yml` (собранный фронт).
