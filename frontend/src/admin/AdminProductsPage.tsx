...
import {
  getProducts, adminGetCategories, adminGetModificationTypes,
    ...
    <div>
      ...
      {showForm && (
        <div className="bg-tg-secondary rounded-2xl p-4 mb-4 space-y-3">
          ...
          {/* Variants (modifications): e.g. Size -> S, M, L with quantities */}
          <div className="space-y-2">
            ...
            {variantModTypeId && (
              <div className="space-y-2 pl-2 border-l-2 border-tg-hint/30">
                <p className="text-xs text-tg-hint">Укажите количество для каждого варианта выбранной модификации:</p>
                ...
                {variantRows.length === 0 && (
                  <p className="text-xs text-tg-hint">
                    У выбранной модификации нет вариантов. Добавьте их в настройках (Модификации).
                  </p>
                )}
              </div>
            )}
              ...
              <div className="grid grid-cols-4 gap-2">
                {pendingFiles.map((file, index) => (
                  <div key={index} className="relative aspect-square rounded-lg overflow-hidden bg-gray-100 group">
                    ...
                    <div className="absolute bottom-1 left-1 text-[10px] bg-blue-500/80 text-white px-1 rounded">
                      Новый
                    </div>
                  </div>
                ))}
            ...
            <div
              ...
              className={`border-2 border-dashed rounded-xl p-4 text-center cursor-pointer transition-colors ${
                dragOver
                  ...
                  : 'border-tg-hint/30 hover:border-tg-button/50'
              }`}
            >
              <input
                ref={fileInputRef}
              ...
              />
              {uploading ? (
                <div className="text-sm text-tg-hint">Загрузка...</div>
              ) : (
                <div className="space-y-1">
                  <Upload className="w-6 h-6 text-tg-hint mx-auto" />
      ...
      <div className="space-y-2">
        {products.map((p) => {
          ...
          const totalStock = p.variants?.length
            ...
            : p.stock_quantity;
          return (
            <div key={p.id} className="bg-tg-secondary rounded-xl p-3 flex items-center gap-3">
